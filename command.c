...
noreturn void external_command(char **argv) {
  ...
  if (!index(argv[0], '/') && path) {
    ...
    for (size_t position; *path != 0; path += ++position) {
      /* Find ':' in path and copy everything before ':' */
      position = strcspn(path, ":");
      char *current_path = strndup(path, position);
      ...
      /* Execute command */
      (void)execve(current_path, argv, environ);
      free(current_path);
    }
#endif /* !STUDENT */
  } else {
